                        ...
                        LPCWSTR fileName)
{
  QString key;
  if (appName == nullptr) {
    key = QString::fromStdWString(keyName);
  } else {
    key = QStringLiteral("%1/%2").arg(appName, keyName);
  }

  return WriteRegistryValue(key, QString::fromStdWString(value),
    ...
    case ERROR_ACCESS_DENIED: {
      ...
      if ((attrs != INVALID_FILE_ATTRIBUTES) && (attrs & FILE_ATTRIBUTE_READONLY)) {
        ...
        if (result & (QMessageBox::Yes | QMessageBox::Ignore)) {
          ...
          if (::SetFileAttributes(wFileName, attrs)) {
            settings.setValue(key, value);
            settings.sync();
        ...
        if (result == QMessageBox::Ignore) {
          attrs |= FILE_ATTRIBUTE_READONLY;
          ::SetFileAttributes(wFileName, attrs);
        }
      }
    } break;
    }
